...
function isClientSnapshot(
  snap: QueryDocumentSnapshot | adminFirestore.QueryDocumentSnapshot
...
export class Animal {
  ...
  static fromFirestore(
    snapshot: QueryDocumentSnapshot | adminFirestore.QueryDocumentSnapshot,
...
export const snapshotChanges = (
  ...
  if (admin) {
    ...
    return from(zone.runOutsideAngular(() => animalsCollection.get())).pipe(
      ...
      map((it) =>
        it.docs.map((change) => change.data()).filter((it) => it._VALID)
      )
    );
  } else {
    ...
    const base$: Observable<any> = bundle
      ? new Observable<LoadBundleTaskProgress>((sub) => {
        ...
        }).pipe(filter((it) => it.taskState === 'Success'))
      : of(undefined);
    return base$.pipe(
...
export const newAnimal = async () => {
  ...
  if (!name) {
    throw new Error('that\'s no name');
  }
