...
const UMD_NAMES = MODULES.map((m) =>
  m === 'core'
    ? 'mandobridge-angularfire'
    : `mandobridge-angularfire-${m.replace('/', '-')}`
);
const ENTRY_NAMES = MODULES.map((m) =>
  m === 'core' ? '@mandobridge/angularfire' : `@mandobridge/angularfire/${m}`
);
...
function zoneWrapExports() {
  const reexport = async (
    ...
    const toBeExported: Array<[string, string, boolean]> = exports
      .filter(
        (it) =>
          !it.startsWith('_') &&
          overrides[it] !== null &&
          overrides[it]?.override !== true
      )
      .map((importName) => {
        const zoneWrap =
          typeof imported[importName] === 'function' &&
          (overrides[importName]?.zoneWrap ??
            importName[0] !== importName[0].toUpperCase());
    ...
    const overridden = Object.keys(overrides).filter(
      (key) => overrides[key]?.override
    );
    await writeFile(
      join(process.cwd(), 'src', `${module}/${name}.ts`),
      `// DO NOT MODIFY, this file is autogenerated by tools/build.ts
${path.startsWith('firebase/') ? `export * from '${path}';\n` : ''}${
        zoneWrapped.length > 0
          ? `import { ɵzoneWrap } from '@mandobridge/angularfire';
import {
  ${zoneWrapped
    .map(([importName]) => `${importName} as _${importName}`)
    .join(',\n  ')}
} from '${path}';
`
          : ''
  ...
  ${rawExport
    .map(
      ([importName, exportName]) =>
        `${importName}${exportName === importName ? '' : `as ${exportName}`}`
    )
    .join(',\n  ')}
} from '${path}';
`
...
${zoneWrapped
  .map(
    ([importName, exportName]) =>
      `export const ${exportName} = ɵzoneWrap(_${importName}, ${
        overrides[importName]?.blockUntilFirst ?? true
      });`
  )
  .join('\n')}
`
  ...
  return Promise.all([
    reexport(
      ...
      'firebase',
      'firebase/analytics',
      tsKeys<typeof import('firebase/analytics')>(),
      {
    ...
    reexport(
      ...
      'firebase',
      'firebase/app',
      tsKeys<typeof import('firebase/app')>()
    ),
    reexport(
      'app-check',
      'firebase',
      'firebase/app-check',
      tsKeys<typeof import('firebase/app-check')>()
    ),
    reexport(
      'auth',
      'rxfire',
      'rxfire/auth',
      tsKeys<typeof import('rxfire/auth')>()
    ),
    reexport(
      'auth',
      'firebase',
      'firebase/auth',
      tsKeys<typeof import('firebase/auth')>(),
      {
        debugErrorMap: null,
    ...
    reexport(
      ...
      'rxfire',
      'rxfire/database',
      tsKeys<typeof import('rxfire/database')>()
    ),
    reexport(
      'database',
      'firebase',
      'firebase/database',
      tsKeys<typeof import('firebase/database')>()
    ),
    reexport(
      'firestore',
      'rxfire',
      'rxfire/firestore',
      tsKeys<typeof import('rxfire/firestore')>(),
      {
    ...
    reexport(
      ...
      'firebase',
      'firebase/firestore',
      tsKeys<typeof import('firebase/firestore')>()
    ),
    reexport(
      'functions',
      'rxfire',
      'rxfire/functions',
      tsKeys<typeof import('rxfire/functions')>(),
      {
    ...
    reexport(
      ...
      'firebase',
      'firebase/functions',
      tsKeys<typeof import('firebase/functions')>()
    ),
    reexport(
      'messaging',
      'firebase',
      'firebase/messaging',
      tsKeys<typeof import('firebase/messaging')>(),
      {
    ...
    reexport(
      'remote-config',
      'rxfire',
      'rxfire/remote-config',
      tsKeys<typeof import('rxfire/remote-config')>(),
      {
    ...
    reexport(
      ...
      tsKeys<typeof import('firebase/remote-config')>(),
      {
        isSupported: { override: true },
      }
    ),
    reexport(
      ...
      'rxfire',
      'rxfire/storage',
      tsKeys<typeof import('rxfire/storage')>(),
      {
        getDownloadURL: null,
        getMetadata: null,
    ...
    reexport(
      ...
      'firebase',
      'firebase/storage',
      tsKeys<typeof import('firebase/storage')>()
    ),
    reexport(
      'performance',
      'rxfire',
      'rxfire/performance',
      tsKeys<typeof import('rxfire/performance')>(),
      {
        getPerformance$: null,
        trace: null,
      }
    ...
    reexport(
      ...
      tsKeys<typeof import('firebase/performance')>()
    ),
    reexport(
      'firestore/lite',
      'rxfire',
      'rxfire/firestore/lite',
      tsKeys<typeof import('rxfire/firestore/lite')>(),
      {
        doc: { exportName: 'docSnapshots' },
        collection: { exportName: 'collectionSnapshots' },
      }
    ...
    reexport(
      ...
      tsKeys<typeof import('firebase/firestore/lite')>()
    ),
  ]);
}
...
function proxyPolyfillCompat() {
  ...
  return Promise.all(
    Object.keys(defaultObject).map((module) =>
      writeFile(
        join(process.cwd(), 'src', `${module}/base.ts`),
        `// DO NOT MODIFY, this file is autogenerated by tools/build.ts
...
const dest = (...args: string[]) =>
  join(process.cwd(), 'dist', 'packages-dist', ...args);
...
function spawnPromise(command: string, args: string[]) {
  return new Promise((resolve) =>
    spawn(command, args, { stdio: 'inherit' }).on('close', resolve)
  );
}
...
async function compileSchematics() {
  ...
  return Promise.all([
    copy(
      ...
      dest('schematics', 'builders.json')
    ),
    copy(
      src('schematics', 'collection.json'),
      dest('schematics', 'collection.json')
    ),
    copy(
      src('schematics', 'migration.json'),
      dest('schematics', 'migration.json')
    ),
    copy(
      src('schematics', 'deploy', 'schema.json'),
      dest('schematics', 'deploy', 'schema.json')
    ),
    copy(
      src('schematics', 'add', 'schema.json'),
      dest('schematics', 'add', 'schema.json')
    ),
    copy(
      src('schematics', 'setup', 'schema.json'),
      dest('schematics', 'setup', 'schema.json')
    ),
...
async function fixImportForLazyModules() {
  await Promise.all(
    LAZY_MODULES.map(async (module) => {
      const packageJson = JSON.parse(
        (await readFile(dest(module, 'package.json'))).toString()
      );
      const entries = Array.from(
        new Set(
          Object.values(packageJson).filter(
      ...
      // TODO fix in Windows
      entries.push(
        `../${module.includes('/') ? '../' : ''}esm2015/${module}/public_api.js`
      );
      await Promise.all(
        entries.map(async (path) => {
          ...
          if (path.endsWith('.umd.js')) {
            ...
            newSource = source.replace(
              new RegExp(`^import 'firebase/${module}'.+$`, 'gm'),
              ''
            );
          }
  ...
  try {
    ...
    const entries = await Promise.all(
      MODULES.map(async (module) => {
        ...
        const entryPoint = typedoc.children.find(
          (c: any) => c.name === '"public_api"'
        );
        const allChildren = [].concat(
          ...typedoc.children.map((child) =>
            // TODO chop out the working directory and filename
            child.children
              ? child.children.map((c) => {
                  return {
                    ...c,
                    path: dirname(child.originalName.split(process.cwd())[1]),
                  };
                })
              : []
          )
        ...
        return (entryPoint.children || [])
          ...
          .map((child) => ({
            ...allChildren.find((c) => child.target === c.id),
          }))
          .reduce(
            (acc, child) => ({
              ...acc,
    ...
    const tocType = (child) => {
      const decorators: string[] =
        (child.decorators && child.decorators.map((d) => d.name)) || [];
      if (decorators.includes('NgModule')) {
        ...
        child.defaultValue &&
        child.defaultValue.startsWith('new InjectionToken')
      ) {
        ...
        pipes.includes(child.signatures[0].type.name)
      ) {
        return 'Pipe';
      } else {
    ...
    const tableOfContents = entries.reduce(
      (acc, entry, index) => ({
        ...acc,
        [MODULES[index]]: {
          name: ENTRY_NAMES[index],
          exports: Object.keys(entry).reduce(
            (acc, key) => ({ ...acc, [key]: tocType(entry[key]) }),
            {}
          ),
        },
      }),
    ...
    const afdoc = entries.reduce(
      (acc, entry, index) => ({ ...acc, [MODULES[index]]: entry }),
      { table_of_contents: tableOfContents }
    );
    return writeFile(
      join(process.cwd(), `api-${root.version}.json`),
      JSON.stringify(afdoc, null, 2)
    );
  } catch (e) {
    console.warn(e);
...
Promise.all([buildDocs(), buildLibrary()])
  .then(measureLibrary)
  .then((stats) =>
    console.log(`
...
------------------------------------
${stats
  .map((s, i) => [MODULES[i].padEnd(21), s.size.padEnd(8), s.gzip].join(''))
  .join('\n')}`)
  );
