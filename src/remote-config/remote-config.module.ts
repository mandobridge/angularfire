...
export const PROVIDED_REMOTE_CONFIG_INSTANCES = new InjectionToken<
  RemoteConfig[]
>('angularfire2.remote-config-instances');
...
export function defaultRemoteConfigInstanceFactory(
  ...
  const defaultRemoteConfig = ɵgetDefaultInstanceOf<FirebaseRemoteConfig>(
    REMOTE_CONFIG_PROVIDER_NAME,
    provided,
    defaultApp
  );
...
export function remoteConfigInstanceFactory(
  fn: (injector: Injector) => FirebaseRemoteConfig
) {
