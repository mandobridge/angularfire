...
import {
  AppCheck,
  AppCheckInstances,
  APP_CHECK_PROVIDER_NAME,
} from './app-check';
import { FirebaseApps, FirebaseApp } from '@mandobridge/angularfire/app';
import { registerVersion } from 'firebase/app';
import { isPlatformServer } from '@angular/common';

export const PROVIDED_APP_CHECK_INSTANCES = new InjectionToken<AppCheck[]>(
  'angularfire2.app-check-instances'
);
export const APP_CHECK_NAMESPACE_SYMBOL = Symbol(
  'angularfire2.app-check.namespace'
);

export function defaultAppCheckInstanceFactory(
  provided: FirebaseAppCheck[] | undefined,
  defaultApp: FirebaseApp
) {
  const defaultAppCheck = ɵgetDefaultInstanceOf<FirebaseAppCheck>(
    APP_CHECK_PROVIDER_NAME,
    provided,
    defaultApp
  );
...
export function appCheckInstanceFactory(
  fn: (injector: Injector) => FirebaseAppCheck
) {
...
export function provideAppCheck(
  fn: (injector: Injector) => FirebaseAppCheck,
  ...deps: any[]
): ModuleWithProviders<AppCheckModule> {
  return {
