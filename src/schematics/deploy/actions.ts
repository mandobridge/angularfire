...
const findPackageVersion = (packageManager: string, name: string) => {
  const match = execSync(`${packageManager} list ${name}`)
    .toString()
    .match(`[^|\s]${escapeRegExp(name)}[@| ][^\s]+(\s.+)?$`);
  return match
    ? match[0].split(new RegExp(`${escapeRegExp(name)}[@| ]`))[1].split(/\s/)[0]
    : null;
};
...
const getPackageJson = (
  ...
  if (existsSync(join(workspaceRoot, 'angular.json'))) {
    ...
    // tslint:disable-next-line:no-non-null-assertion
    const server =
      angularJson.projects[(context.target as Target).project].architect.server;
...
export const deployToFunction = async (
  ...
  const staticBuildOptions = await context.getTargetOptions(
    targetFromTargetString(staticBuildTarget.name)
  );
  if (
    !staticBuildOptions.outputPath ||
    typeof staticBuildOptions.outputPath !== 'string'
  ) {
    throw new Error(
  ...
  const serverBuildOptions = await context.getTargetOptions(
    targetFromTargetString(serverBuildTarget.name)
  );
  if (
    !serverBuildOptions.outputPath ||
    typeof serverBuildOptions.outputPath !== 'string'
  ) {
    throw new Error(
  ...
  if (options.preview) {
    ...
    const { deployProject } = await inquirer.prompt({
      ...
      name: 'deployProject',
      message:
        'Would you like to deploy your application to Firebase Hosting & Cloud Functions?',
    });
...
export const deployToCloudRun = async (
  ...
  const packageJson = getPackageJson(
    ...
    workspaceRoot,
    options,
    join(serverBuildOptions.outputPath, 'main.js')
  );
  const nodeVersion = packageJson.engines.node;
  ...
  fsHost.writeFileSync(join(cloudRunOut, 'Dockerfile'), dockerfile(options));
  ...
  await spawnAsync(
    `gcloud builds submit ${cloudRunOut} --tag gcr.io/${options.firebaseProject}/${serviceId} --project ${options.firebaseProject} --quiet`
  );
  await spawnAsync(
    `gcloud run deploy ${serviceId} --image gcr.io/${
      options.firebaseProject
    }/${serviceId} --project ${options.firebaseProject} ${deployArguments.join(
      ' '
    )} --platform managed --allow-unauthenticated --region=${
      options.region
    } --quiet`
  );
  ...
  const logger = new winston.transports.Console({
    ...
    format: winston.format.printf((info) => {
      ...
      if (text?.replace) {
        ...
        if (
          emulator?.name === 'hosting' &&
          plainText.startsWith('Local server: ')
        ) {
