...
const IMPORT_REGEX =
  /(?<key>import|export)\s+(?:(?<alias>[\w,{}\s\*]+)\s+from)?\s*(?:(?<quote>["'])?(?<ref>[@\w\s\\\/.-]+)\3?)\s*(?<term>[;\n])/g;
...
export const ngUpdate = (): Rule => (host: Tree, context: SchematicContext) => {
  const packageJson =
  ...
  Object.keys(peerDependencies).forEach((depName) => {
    ...
    if (dep) {
      packageJson[dep.dev ? 'devDependencies' : 'dependencies'][depName] =
        dep.version;
    }
  });
  ...
  const angularJson =
    host.exists('angular.json') && safeReadJSON('angular.json', host);
  if (packageJson === undefined) {
    throw new SchematicsException('Could not locate angular.json');
  }

  // TODO investigate if this is correct in Windows
  const srcRoots: string[] = Object.values(angularJson.projects).map(
    (it: any) => join(...['/', it.root, it.sourceRoot].filter((it) => !!it))
  );
  ...
  host.visit((filePath) => {
    ...
    const newContent = content.replace(IMPORT_REGEX, (substring, ...args) => {
      ...
      if (
        ref.startsWith('@mandobridge/angularfire') &&
        !ref.startsWith('@mandobridge/angularfire/compat')
      ) {
        return `${key} ${alias} from ${quote}${ref.replace(
          '@mandobridge/angularfire',
          '@mandobridge/angularfire/compat'
        )}${quote}${term}`;
      }
