...
export const REGION = new InjectionToken<string>(
  'angularfire2.functions.region'
);

type UseEmulatorArguments = Parameters<
  firebase.functions.Functions['useEmulator']
>;
export const USE_EMULATOR = new InjectionToken<UseEmulatorArguments>(
  'angularfire2.functions.use-emulator'
);

// override httpsCallable for compatibility with 5.x
export interface AngularFireFunctions
  extends Omit<ɵPromiseProxy<firebase.functions.Functions>, 'httpsCallable'> {}

@Injectable({
  providedIn: 'any',
})
export class AngularFireFunctions {
  public readonly httpsCallable: <T = any, R = any>(
    name: string,
  ...
  constructor(
    ...
    const functions = of(undefined).pipe(
      ...
      map((app) =>
        ɵcacheInstance(
          `${app.name}.functions.${region || origin}`,
    ...
    this.httpsCallable =
      <T = any, R = any>(name: string, options?: HttpsCallableOptions) =>
      (data: T) =>
        from(functions).pipe(
          observeOn(schedulers.insideAngular),
          switchMap((functions) =>
            functions.httpsCallable(name, options)(data)
          ),
          map((r) => r.data as R)
        );
