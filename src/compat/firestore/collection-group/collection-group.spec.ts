...
async function collectionHarness(
  afs: AngularFirestore,
  items: number,
  queryGroupFn?: QueryGroupFn<Stock>
) {
  ...
  const stocks = new AngularFirestoreCollectionGroup<Stock>(
  ...
  describe('valueChanges()', () => {
    ...
    it('should handle dynamic queries that return empty sets', (done) => {
      (async () => {
        ...
        const ref = afs.firestore.collection(`${randomCollectionName}`);
        const names = await createRandomStocks(afs.firestore, ref, ITEMS);
        const sub = pricefilter$
          .pipe(
            switchMap((price) => {
              return afs
                .collection(randomCollectionName, (ref) =>
                  price ? ref.where('price', '==', price) : ref
                )
                .valueChanges();
            })
          )
  ...
  describe('snapshotChanges()', () => {
    ...
    it('should update order on queries', (done) => {
      (async () => {
        ...
        let count = 0;
        let firstIndex = 0;
        const { ref, stocks, names } = await collectionHarness(
          afs,
          ITEMS,
        ...
        const sub = stocks.snapshotChanges().subscribe((data) => {
          ...
          if (count === 1) {
            // make an update
            firstIndex = data.filter((d) => d.payload.doc.id === names[0])[0]
              .payload.newIndex;
            ref.doc(names[0]).update({ price: 2 });
          }
          // on the second round, make sure the array is still the same
    ...
    it('should be able to filter snapshotChanges() types - added', (done) => {
      (async () => {
        ...
        delayAdd(ref.doc(names[0]).collection(randomCollectionName), nextId, {
          price: 2,
        });
      })();
    });
    ...
    it('should be able to filter snapshotChanges() types - added w/same id', (done) => {
      (async () => {
        ...
        const { randomCollectionName, ref, stocks, names } =
          await collectionHarness(afs, ITEMS);
        ...
        delayAdd(ref.doc(names[0]).collection(randomCollectionName), names[0], {
          price: 3,
        });
      })();
    });
