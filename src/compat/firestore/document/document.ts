...
export class AngularFirestoreDocument<T = DocumentData> {
   ...
   * function.
   */
  collection<R = DocumentData>(
    path: string,
    queryFn?: QueryFn
  ): AngularFirestoreCollection<R> {
    const collectionRef = this.ref.collection(
      path
    ) as firebase.firestore.CollectionReference<R>;
  ...
  snapshotChanges(): Observable<Action<DocumentSnapshot<T>>> {
    const scheduledFromDocRef$ = fromDocRef<T>(
      this.ref,
  ...
  valueChanges<K extends string>(options: {
    idField: K;
  }): Observable<(T & { [T in K]: string }) | undefined>;
