...
export const SETTINGS = new InjectionToken<Settings>(
  'angularfire2.remoteConfig.settings'
);
export const DEFAULTS = new InjectionToken<ConfigTemplate>(
  'angularfire2.remoteConfig.defaultConfig'
);

export interface AngularFireRemoteConfig
  extends ɵPromiseProxy<firebase.remoteConfig.RemoteConfig> {}
...
const proxyAll = (
  observable: Observable<Parameter[]>,
  as: 'numbers' | 'booleans' | 'strings'
) =>
  new Proxy(observable.pipe(mapToObject(as as any)), {
...
export class Value implements firebase.remoteConfig.Value {
  asBoolean() {
    return (
      ['1', 'true', 't', 'y', 'yes', 'on'].indexOf(this._value.toLowerCase()) >
      -1
    );
  }
  ...
  constructor(
    public _source: firebase.remoteConfig.ValueSource,
...
export class Parameter extends Value {
  constructor(
    public key: string,
    public fetchTimeMillis: number,
    source: firebase.remoteConfig.ValueSource,
    value: string
  ) {
    super(source, value);
  }
...
const filterTest = (fn: (param: Parameter) => boolean) =>
  filter<Parameter | Parameter[]>((it) =>
    Array.isArray(it) ? it.some(fn) : fn(it)
  );
...
export const filterFresh = (howRecentInMillis: number) =>
  filterTest(
    ...
    scan((existing, [all, rc]) => {
      ...
      return allKeys.map((key) => {
        ...
        return updatedValue
          ? new Parameter(
              key,
              rc ? rc.fetchTimeMillis : -1,
              updatedValue.getSource(),
              updatedValue.asString()
            )
...
export class AngularFireRemoteConfig {
  ...
  readonly numbers: Observable<{ [key: string]: number | undefined }> & {
    [key: string]: Observable<number>;
  };
  readonly booleans: Observable<{ [key: string]: boolean | undefined }> & {
    [key: string]: Observable<boolean>;
  };
  readonly strings: Observable<{ [key: string]: string | undefined }> & {
  ...
  constructor(
    ...
    const remoteConfig$ = of(undefined).pipe(
      ...
      switchMap((isSupported) =>
        isSupported ? import('firebase/compat/remote-config') : EMPTY
      ),
    ...
    const default$: Observable<{ [key: string]: firebase.remoteConfig.Value }> =
      of(
    ...
    this.changes = this.parameters.pipe(
      ...
      mergeMap((group) => group.pipe(distinctUntilChanged()))
    );
...
export const budget =
  <T>(interval: number): MonoTypeOperatorFunction<T> =>
  (source: Observable<T>) =>
...
export function scanToObject(): OperatorFunction<
  Parameter,
  { [key: string]: string | undefined }
>;
export function scanToObject(
  to: 'numbers'
): OperatorFunction<Parameter, { [key: string]: number | undefined }>;
export function scanToObject(
  to: 'booleans'
): OperatorFunction<Parameter, { [key: string]: boolean | undefined }>;
// tslint:disable-next-line:unified-signatures
export function scanToObject(
  to: 'strings'
): OperatorFunction<Parameter, { [key: string]: string | undefined }>;
export function scanToObject<T extends ConfigTemplate>(
  template: T
): OperatorFunction<Parameter, T & { [key: string]: string | undefined }>;
export function scanToObject<T extends ConfigTemplate>(
  to: 'numbers' | 'booleans' | 'strings' | T = 'strings'
) {
  return pipe(
    // TODO cleanup
    scan(
...
export function mapToObject(): OperatorFunction<
  Parameter[],
  { [key: string]: string | undefined }
>;
...
export function mapToObject<T extends ConfigTemplate>(
  template: T
): OperatorFunction<Parameter[], T & { [key: string]: string | undefined }>;
export function mapToObject<T extends ConfigTemplate>(
  to: 'numbers' | 'booleans' | 'strings' | T = 'strings'
