...
export const USE_EMULATOR = new InjectionToken<UseEmulatorArguments>(
  'angularfire2.auth.use-emulator'
);

export const SETTINGS = new InjectionToken<firebase.auth.AuthSettings>(
  'angularfire2.auth.settings'
);
export const TENANT_ID = new InjectionToken<string>(
  'angularfire2.auth.tenant-id'
);
export const LANGUAGE_CODE = new InjectionToken<string>(
  'angularfire2.auth.langugage-code'
);
export const USE_DEVICE_LANGUAGE = new InjectionToken<boolean>(
  'angularfire2.auth.use-device-language'
);
export const PERSISTENCE = new InjectionToken<string>(
  'angularfire.auth.persistence'
);

export const ɵauthFactory = (
  app: FirebaseApp,
  zone: NgZone,
  useEmulator: UseEmulatorArguments | null,
  tenantId: string,
  languageCode: string | null,
  useDeviceLanguage: boolean | null,
  settings: firebase.auth.AuthSettings | null,
...
export class AngularFireAuth {
  ...
  constructor(
    ...
    const auth = of(undefined).pipe(
      ...
      switchMap(() =>
        zone.runOutsideAngular(() => import('firebase/compat/auth'))
      ),
    ...
    if (isPlatformServer(platformId)) {
      ...
      const authStateChanged = auth.pipe(
        switchMap(
          (auth) =>
            new Observable<firebase.User | null>((sub) => ({
      ...
      this.credential = merge(
        ...
        map((credential) =>
          credential?.user
            ? (credential as Required<firebase.auth.UserCredential>)
            : null
        ),
        subscribeOn(schedulers.outsideAngular),
        observeOn(schedulers.insideAngular)
      ...
      spy: {
        apply: (name, _, val) => {
          ...
          if (name.startsWith('signIn') || name.startsWith('createUser')) {
            ...
            val.then((user: firebase.auth.UserCredential) =>
              logins.next(user as any)
            );
          }
        },
      },
