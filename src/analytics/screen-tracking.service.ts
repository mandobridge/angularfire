...
const getScreenInstanceID = (params: { [key: string]: any }) => {
  ...
  const screenInstanceKey = [params[SCREEN_CLASS_KEY], params[OUTLET_KEY]].join(
...
export const ɵscreenViewEvent = (
  ...
  const activationEndEvents = router.events.pipe(
    filter<ActivationEnd>((e) => e instanceof ActivationEnd)
  );
    ...
    switchMap<ActivationEnd, Observable<Record<string, any> | null>>(
      (activationEnd) => {
        ...
        const urlTree = router.parseUrl(
          router.url.replace(/(?:\().+(?:\))/g, (a) => a.replace('://', ':///'))
        );
        const pagePath =
        ...
        map(([prior, current]) =>
          prior
            ? {
                ...
                [FIREBASE_PREVIOUS_SCREEN_INSTANCE_ID_KEY]:
                  prior[FIREBASE_SCREEN_INSTANCE_ID_KEY],
                ...current,
              }
            : current
        )
